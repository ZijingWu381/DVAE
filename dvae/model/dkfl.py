...
def build_DKFL(cfg, device='cpu'):

    ### Load parameters
    # General
    x_dim = cfg.getint('Network', 'x_dim')
    z_dim = cfg.getint('Network','z_dim')
    lag = cfg.getint('Network','lag')
...
def stair_zero_pad(x, lags):
    ...
    for lag in range(1, lags + 1):
        x_0 = torch.zeros(lag, *x.shape[1:])
        x_tmp = torch.cat([x_0, x[:-lag, :, :]], 0)
...
class DKF(nn.Module):
    ...
    def build(self):
        ...
        dic_layers['linear2'] = nn.Linear(gen_z_dim, self.z_dim)
        ...
        # 3. Prior
        self.prior_mean = nn.Linear(gen_z_dim, self.z_dim)
    ...
    def generation_z(self, z_prev):

        gate = self.mlp_gate(z_prev)
        z_prop = self.mlp_z_prop(z_prev)
        z_mean_p = (1 - gate) * self.prior_mean(z_prev) + gate * z_prop
